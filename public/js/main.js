...
class Bingo {
    ...
    save_numbers() {
        ...
        if (get_item == null) {
            ...
            // console.log(this.grid[0][0].innerHTML);
        } else {
            //--------------------------------------------------//
            // let get_item = localStorage.getItem("grid");

            let t = JSON.parse(localStorage.getItem("grid"));
            // this.add_event_click_to_btn(t);
            ...
            // while (t.length) this.grid.push(this.temp.splice(0, 5));
        }
    }
        ...
        document.querySelector(".bingo-btn").addEventListener(
            ...
            () => {
                ...
                for (let row = 0; row < this.grid.length; row++) {
                    ...
                    let count = 0;
                    while (count < 5) {
                        if (this.grid[row][count].getAttribute("val") == "1" && found == 5) {
                ...
                for (let row = 0; row < this.grid.length; row++) {
                    ...
                    let count = 0;
                    while (count < 5) {
                        if (this.grid[count][row].getAttribute("val") == "1" && found == 5) {
...
document.querySelector("#username-btn").addEventListener("click", () => {
    let username = document.querySelector("#username").value.match("[_a-zA-Z0-9ÅÄÖåäö]+");
